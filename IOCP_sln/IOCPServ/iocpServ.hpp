...
public:
	int m_RunFlag;
	CPCQueue<PPER_IO_DATA> m_queueRecv;
...
CIocpServ()
{
	...
	if( 0 != WSAStartup( MAKEWORD( 2, 2 ), &wsaData ) )
	{
		...
		printf( "with API versions: %d.%d to %d.%d",LOBYTE( wsaData.wVersion), HIBYTE( wsaData.wVersion ),LOBYTE( wsaData.wHighVersion), HIBYTE( wsaData.wHighVersion) );
		return;
	}
...
int IocpRun(const char *pszAddr, int nPort)
{
  ...
  // 创建工作线程处理完成端口对象的事件
  printf("IoCompletionPort work thread num = %d\n",nProcessors);
  for (int i=0; i<nProcessors; i++)
  {
    HANDLE hThread;
...
int SendAnsData(PPER_HANDLE_DATA pPerHandle, PPER_IO_DATA pPerIo, std::string &strAnsData)
{
  WSABUF wsabuf;
  DWORD dwSend;
