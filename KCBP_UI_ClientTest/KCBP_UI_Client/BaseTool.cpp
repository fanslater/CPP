...
tstring CBaseTool::GetTimestamp_AsStr()
{
    char szTimestamp[32] = {0};
    SYSTEMTIME st = {0};
    GetLocalTime( &st );
    sprintf_s( szTimestamp, "%04d%02d%02d%02d%02d%02d%03d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds );
    return tstring( szTimestamp );
}
...
tstring CBaseTool::get_json_val( const Json::Value& json, const tstring& key, bool bNeedTrim )
{
    tstring strRet = "";
