...
int CAgreementClient::tcp_sync_socket(IN const std::string &strUrl, IN const std::string &strSend, IN int iConnectTimeOut, IN int iTransmitTimeOut, OUT std::string &strResponse)
{
    CURL *curlHandle;
    CURLcode code;
    ...
    if (curlHandle)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                nsent_total += nsent;
                if(code == CURLE_AGAIN && !wait_on_socket(sockfd, 0, 1000L * iConnectTimeOut))
                {
                    return -1;
                }
            }
        ...
        while(1)
        {
            ...
            do
            {
                ...
                if(code == CURLE_AGAIN && !wait_on_socket(sockfd, 1, 1000L * iTransmitTimeOut))
                {
                    return -1;
                }
            }
