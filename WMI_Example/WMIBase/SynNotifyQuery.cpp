#include "SynNotifyQuery.h"

#include <atlbase.h>
#include <atlcom.h>

CSynEventNotify::CSynEventNotify(const wstring& wszNamespace,const wstring& wszWQLQuery)
{
    SetNamespace(wszNamespace);
    m_wszWQLQuery = wszWQLQuery;
}

CSynEventNotify::~CSynEventNotify(void)
{
}

HRESULT CSynEventNotify::Excute( CComPtr<IWbemServices> pSvc )
{
...
HRESULT CSynEventNotify::DealWithIWbemClassObject( CComPtr<IWbemClassObject> pclsObj )
{
...
HRESULT CSynEventNotify::DealWithSingleItem( CComBSTR bstrName, CComVariant Value, CIMTYPE type, LONG lFlavor )
{   
